...
const BadgeStyles = {
  bestseller:
    "bg-gradient-to-r from-yellow-400/20 to-amber-400/20 text-yellow-200 border-yellow-400/30 shadow-sm",
  trending:
    "bg-gradient-to-r from-blue-400/20 to-cyan-400/20 text-blue-200 border-blue-400/30 shadow-sm",
  popular:
    "bg-gradient-to-r from-green-400/20 to-emerald-400/20 text-green-200 border-green-400/30 shadow-sm",
  featured:
    "bg-gradient-to-r from-purple-400/20 to-fuchsia-400/20 text-purple-200 border-purple-400/30 shadow-sm",
};
...
export default function EnhancedCourseCard({
  ...
  return (
    ...
    >
      <div
        className={`relative transform-gpu transition-all duration-500 h-[450px] w-full max-w-[380px] mx-auto group`}
      >
        ...
        <div className="relative h-full w-full bg-gradient-to-b from-zinc-900/90 to-black/90 backdrop-blur-xl rounded-2xl overflow-hidden transition-all duration-300 hover:translate-y-[-4px]">
          ...
          <div className="absolute top-3 left-3 right-3 z-10 flex flex-wrap gap-2">
            {course.isBestseller && (
              <Badge
                variant="secondary"
                className={`transition-all scale-100 group-hover:scale-105 ${BadgeStyles.bestseller}`}
              >
            ...
            {course.isTrending && (
              <Badge
                variant="secondary"
                className={`transition-all scale-100 group-hover:scale-105 ${BadgeStyles.trending}`}
              >
            ...
            {course.isPopular && (
              <Badge
                variant="secondary"
                className={`transition-all scale-100 group-hover:scale-105 ${BadgeStyles.popular}`}
              >
            ...
            {course.isFeatured && (
              <Badge
                variant="secondary"
                className={`transition-all scale-100 group-hover:scale-105 ${BadgeStyles.featured}`}
              >
          ...
          <div className="relative h-[200px] overflow-hidden">
            <Image
              src={
                course.thumbnail
                  ? `${process.env.NEXT_PUBLIC_IMAGE_URL}/${course.thumbnail}`
                  : "/placeholder.jpeg"
              }
              alt={course.title}
              fill
          ...
          <div className="relative p-5 space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs">
              <Badge
                variant="outline"
                className="bg-black/30 text-green-400 border-green-500/30 shadow-sm uppercase px-2 py-0.5"
              >
              ...
              {hidePrice ? (
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between">
                    <Badge
                      variant="secondary"
                      className="bg-gradient-to-r from-green-500 to-emerald-600 text-white border-0 shadow-sm px-2.5 py-1"
                    >
          ...
          <div
            className={`absolute inset-0 bg-gradient-to-t from-green-950/40 via-black/20 to-transparent 
